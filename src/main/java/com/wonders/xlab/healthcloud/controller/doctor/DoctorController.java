...
public class DoctorController extends AbstractBaseController<Doctor, Long> {
                ...
                if (!iden_code.equals(iden.getCode())) {
                    ...
                    if (doctor == null) { // 如果是新用户，插入记录
                        ...
                        return new ControllerResult<Doctor>()
                                .setRet_code(0).setRet_values(doctor)
                                .setMessage("成功");
                    } else {
                        return new ControllerResult<Doctor>()
                                .setRet_code(0).setRet_values(doctor)
        ...
        try {
            ...
            if (token.getTel() == null) {
                ...
                if (third == null) {
                    ...
                    // 有第三方账号，返回医师id
                    return new ControllerResult<Doctor>()
                            .setRet_code(0).setRet_values(third.getDoctor())
                            .setMessage("成功");
                }
            } else {
                if (third != null) {
    ...
    public Object uploadPic(@PathVariable long id, @RequestParam("file") MultipartFile file) throws Exception {
        ...
        try {
            ...
            return new ControllerResult<String>()
                    .setRet_code(0).setRet_values(url)
                    .setMessage("成功");
        } catch (Exception e) {
            e.printStackTrace();
            return new ControllerResult<String>()
